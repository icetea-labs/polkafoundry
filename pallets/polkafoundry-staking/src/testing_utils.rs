	...
	-> Result<(T::AccountId, T::AccountId), &'static str>
{
	...
	Staking::<T>::bond(RawOrigin::Signed(stash.clone()).into(), controller, amount, destination)?;
...
pub fn create_validators<T: Config>(
	...
	let mut validators: Vec<T::AccountId> = Vec::with_capacity(max as usize);
	for i in 0 .. max {
...
pub fn create_validators_with_nominators_for_era<T: Config>(
	...
	for i in 0 .. validators {
		...
		validators_stash.push(stash.clone());
	}
	...
	for j in 0 .. nominators {
		...
		Staking::<T>::nominate(RawOrigin::Signed(n_controller.clone()).into(), selected_validators, balance_factor)?;
	}
