...
pub(crate) fn two_deferred_slash() -> sp_io::TestExternalities {
	ExtBuilder::default()
...
pub(crate) fn bond_nominator(
	...
	let _ = Balances::make_free_balance_be(&ctrl, val);
	assert_ok!(Staking::nominate(Origin::signed(ctrl), target, val));
}
...
pub(crate) fn add_slash(who: &AccountId) {
	on_offence_now(
		&[
			OffenceDetails {
				offender: (who.clone(), Staking::eras_stakers_clipped(active_era(), who.clone())),
				reporters: vec![],
			},
		],
