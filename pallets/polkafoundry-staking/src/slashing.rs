...
use super::{
	Store,
	EraIndex, Config, Pallet, BalanceOf, Exposure, SessionInterface,
	NegativeImbalanceOf, UnappliedSlash, Error
};
use sp_runtime::{traits::{Zero, Saturating}, RuntimeDebug, DispatchResult};
use frame_support::{
	ensure,
	traits::{Currency, OnUnbalanced, Imbalance, ReservableCurrency, Get},
};
use sp_std::vec::Vec;
									   ...
									   -> Option<UnappliedSlash<T::AccountId, BalanceOf<T>>>
{
	...
	let (prior_slash_p, _era_slash) = <Pallet<T> as Store>::CollatorSlashInEra::get(
		&slash_era,
		stash,
	...
	if slash.deconstruct() > prior_slash_p.deconstruct() {
		<Pallet<T> as Store>::CollatorSlashInEra::insert(
			&slash_era,
			stash,
			Some(&(slash, own_slash)),
		);
	} else {
...
fn kick_out_if_recent<T: Config>(
	...
	if spans.era_span(params.slash_era).map(|s| s.index) == Some(spans.span_index()) {
		...
		<Pallet<T>>::kick_stash(params.stash, spans.spans.iter().count() as u32);
		// make sure to disable validator till the end of this session
...
fn fetch_spans<'a, T: Config + 'a>(
	...
	let spans = <Pallet<T> as Store>::SlashingSpans::get(stash).unwrap_or_else(|| {
		...
		<Pallet<T> as Store>::SlashingSpans::insert(stash, Some(&spans));
		spans
	});
...
impl<'a, T: 'a + Config> Drop for InspectingSpans<'a, T> {
	fn drop(&mut self) {
		...
		<Pallet<T> as Store>::SlashingSpans::insert(self.stash, Some(&self.spans));
	}
}

/// Clear slashing metadata for an obsolete era.
pub(crate) fn clear_era_metadata<T: Config>(obsolete_era: EraIndex) {
	<Pallet<T> as Store>::CollatorSlashInEra::remove_prefix(&obsolete_era);
...
pub fn do_slash<T: Config>(
	...
	if !value.is_zero() {
		...
		<Pallet<T> as Store>::Ledger::insert(&controller, &ledger);
		T::Currency::unreserve(stash, pre_total);
		T::Currency::reserve(stash, ledger.total);

		// trigger the event
		<Pallet<T>>::deposit_event(
			super::Event::Slash(stash.clone(), value)
		);
	}
}

pub fn do_slash_nominator<T: Config>(
	...
	let mut nominator = match <Pallet<T>>::nominators(&stash) {
		Some(n) => n,
		None => return, // nothing to do.
	};

	let controller = match <Pallet<T>>::bonded(&validator) {
		None => return, // defensive: should always exist.
		Some(c) => c,
	};

	let pre_total = nominator.total;
	let era = <Pallet<T>>::current_era().unwrap_or(0) + T::BondingDuration::get();

	let (value, less) = nominator.slash(
		validator,
		value,
	...
	if let Some(less) = less {
		if less < T::MinNominatorStake::get() {
			let mut ledger = match <Pallet<T>>::ledger(&controller) {
				Some(ledger) => ledger,
				None => return, // nothing to do.
			};
			ledger.nominations.retain(|b| b.owner != *stash);
			<Pallet<T> as Store>::Ledger::insert(&controller, &ledger);
		}
	}
	...
	if !value.is_zero() {
		...
		<Pallet<T> as Store>::Nominators::insert(&stash, &nominator);
		T::Currency::unreserve(stash, pre_total);
		T::Currency::reserve(stash, nominator.total);
